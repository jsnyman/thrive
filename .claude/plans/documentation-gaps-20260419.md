...
What follows is verified findings only, ranked by impact, with specific file paths and proposed fixes. This is a *fix list* — no behavior change, doc edits only.
...
- These are *device personas*, not RBAC roles — but the language reads as if they were technical role names
...
- `event_model.md:21` *does* include `recordedAt` in the envelope table.
- `docs/api.md:778, 811-813` *does* document `GET /sync/status`.
- `docs/api.md:952-962` *does* document `STAFF_SEED_JSON` with a usage example.
